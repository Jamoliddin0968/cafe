...
{
    DataModule3->login->SQL->Text = " select * from xodimlar where parol = '" +
                                    LabeledEdit2->Text.Trim() + "' ";
    DataModule3->login->Open();
    if (DataModule3->login->RecordCount != 0) {
        String huquq =
            DataModule3->login->FieldByName("huquq")->AsString.Trim();
        if (huquq.LowerCase() == "admin")
            Form7->ShowModal();
        else if (huquq == "ofitsiant" || huquq == "Ofitsiant") {
            Form4->Label8->Caption =
                DataModule3->login->FieldByName("fish")->AsString;
            Form4->ShowModal();
        }
    }
    LabeledEdit2->Text = "";
